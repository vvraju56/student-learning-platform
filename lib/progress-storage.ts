    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith('video_progress_')) {
        try {
          ...
          // Extract courseId from key format: video_progress_{courseId}_{userId}_{videoId}
          const keyParts = key.split('_')
          if (keyParts.length >= 4 && keyParts[2] === userId) {
            progressData.push({
              ...data,
              courseId: keyParts[1] // keyParts[1] is the courseId
            })
          }
        } catch (error) {
