...
function SmartDashboardContent({ user, profile, handleLogout }: SmartDashboardProps) {
  ...
  useEffect(() => {
    ...
    const loadProgress = async () => {
      try {
        ...
        const userProgress = await firebaseManager.getUserProgress()
        const fetchedCoursesData = userProgress?.courses || {};
          ...
          if (completedVideos === 0) {
            ...
            if (localData) {
              try {
                ...
                console.log(`📺 Loaded ${courseId} progress from localStorage:`, completedVideos)
              } catch (e) {
        ...
        setLearningStats({
          totalHours: 0, // Need to calculate this based on video durations
          completedVideos: totalCompletedCount,
          totalViolations: 0 // TODO: Load from Firebase
        })
        
        console.log(`📊 Loaded progress: Overall ${Math.round(overallProgressValue)}%`)
      } catch (error) {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        console.log('Page hidden - pausing progress updates and timer')
        clearInterval(interval); // Clear the interval when page is hidden
      } else {
        console.log('Page visible - resuming progress updates and timer')
  ...
  useEffect(() => {
    if (isVideoPlaying && focusActive && isMonitoring) {
      console.log('📈 Course progress bar increases live')
      console.log('✅ Overall status updates instantly')
      console.log('👁️ Monitoring status remains active')
    }
  }, [isVideoPlaying, focusActive, isMonitoring])

  useEffect(() => {
    if (!focusActive && isVideoPlaying) {
      console.log('🚫 Focus Lost - Video auto-pauses')
      console.log('⏸️ Progress bar freezes')
      console.log('🔴 Status changes to Distracted')
    }
                    ...
                    <div key={courseFromLib.id} className="col-md-4">
                      <div className="p-3 rounded-lg" style={{ backgroundColor: '#1a1a1a', border: '1px solid #2a2a2a' }}>
                        ...
                        <div className="d-flex justify-content-between align-items-center mb-2">
                          ...
                          <span className={`small fw-bold ${courseDisplayData[courseFromLib.id]?.textColor}`}>
                            {Math.round(courseProgress[courseFromLib.id]?.progress || 0)}%
                          </span>
                        </div>
                        <div className="w-full bg-gray-600 rounded-full h-2">
                          <div
                            className={`h-2 rounded-full transition-all duration-300 bg-${courseDisplayData[courseFromLib.id]?.bgColor}`}
                            style={{ 
